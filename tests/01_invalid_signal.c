...
int invalid_signal_callback(struct pollen_callback *callback, int signum, void *data) {
    return 0;
}
...
int main(void) {
    ...
    /* intentionally invalid signal - should fail */
    assert(pollen_loop_add_signal(loop, 0xDEAD, invalid_signal_callback, NULL) == NULL);
